"""Numba-accelerated batched dynamic-programming smoothing kernels."""
...
from pyosv._dp.path2d_numba import min3_prefer_center
...
def smooth_fault_attributes_batch_into(
    ...
    for ib in range(batch):
        ...
        for ii in range(ni):
            for il in range(nl):
                smoothed[ib, ii, il] = smoothed[ib, ii, il] + reverse[ii, il] - cost[ib, ii, il]
