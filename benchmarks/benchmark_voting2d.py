...
from pyosv._seed_selection import _select_voter_seed_indices_2d  # noqa: E402
...
def main(argv: list[str] | None = None) -> int:
    ...
    ft, pt = synthetic_fault_likelihood(
        args.n2,
        args.n1,
    ...
    seed_times, seeds = time_repeated(
        lambda: voter.pick_seeds(args.d, args.fm, ft, pt),
        repeat=args.repeat,
        warmup=args.warmup,
    )
    threshold = np.float32(args.fm)
    python_seed_times, python_seed_indices = time_repeated(
        lambda: _select_voter_seed_indices_2d(
            ft,
            threshold,
            args.d,
            use_numba=False,
    ...
    print(array_fingerprint("w1", w1))
    print(array_fingerprint("w2", w2))
    return 0
...
def synthetic_fault_likelihood(
    ...
    *,
    ...
    distance = i2 - center2
    structure = np.exp(-0.5 * (distance / np.float32(1.4)) ** 2).astype(np.float32)
    candidates = rng.random((n2, n1)) < candidate_density
    jitter = rng.random((n2, n1), dtype=np.float32)
    ft = np.where(
        candidates,
