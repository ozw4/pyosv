...
def test_writer_creates_complete_valid_bundle_with_stable_headers(tmp_path: Path) -> None:
    ...
    assert manifest["scalar_evidence_contract_version"] == SCALAR_EVIDENCE_CONTRACT_VERSION == 7
...
def test_validator_explicitly_rejects_rehashed_scalar_contract_v3_bundle(
    tmp_path: Path,
) -> None:
    bundle = _write_bundle(tmp_path / "legacy-scalar-v3")
    manifest_path = bundle / "manifest.json"
    manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    manifest["scalar_evidence_contract_version"] = 3
    manifest_path.write_text(
        json.dumps(manifest, separators=(",", ":")) + "\n",
        encoding="utf-8",
        newline="\n",
...
def test_validator_explicitly_rejects_rehashed_runtime_contract_v1_bundle(
    tmp_path: Path,
) -> None:
    bundle = _write_bundle(tmp_path / "legacy-runtime-v1")
    manifest_path = bundle / "manifest.json"
    manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    ...
    with pytest.raises(ValueError, match="legacy schema-v3 bundle.*shared scalar evidence"):
        validate_completed_bundle(bundle)


def test_validator_explicitly_rejects_rehashed_runtime_contract_v2_bundle(
    tmp_path: Path,
) -> None:
    bundle = _write_bundle(tmp_path / "legacy-runtime-v2")
    manifest_path = bundle / "manifest.json"
    manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    manifest["runtime_contract_version"] = 2
    manifest_path.write_text(
        json.dumps(manifest, separators=(",", ":")) + "\n",
        encoding="utf-8",
        newline="\n",
