...
| `samplesInUvwBox(...)` | `samples_in_uvw_box` | Samples `1 - fx` in a seed-centered local UVW box using normal, dip, and strike axes; invalid lag cells stay at default cost and image samples are rounded/clamped. | Returns local costs shaped `(2 * rw + 1, 2 * rv + 1, 2 * ru + 1)`, indexed as `(w, v, u)`, using `lmins` / `lmaxs`, Java-style `floor(x + 0.5)` rounding, and clamping to `(n3, n2, n1)` bounds. | Axis-aligned UVW shape, axis mapping, shift masks, boundary clamping, half-sample rounding, and Python/Numba agreement are covered by local regression tests; no Java fixture covers non-orthogonal axes. | `partially covered` | Add Java-derived fixtures for oblique and out-of-bounds local frames, checking every local `(kw, kv, ku)` cost. |
| `findSurface(float[][][] fx)` | `dp.find_surface_3d` | Repeatedly smooths local costs, solves optimal paths across local slices with strain bounds, and smooths the final surface. | Validates local `(nw, nv, nu)` costs, applies `smooth_fault_attributes_3d`, solves each `w` row with `find_path_2d`, and optionally smooths the `(nw, nv)` surface. | Java DP implementation and smoothing filters differ; Python decomposes the 3D surface into staged 2D helpers. Current reference-audit tests cover flat zero-lag, strain-limited sloped, attribute smoothing on/off, and surface smoothing on/off helper behavior. | `partially covered` | Compare flat, sloped, and tie-heavy cost volumes from Java fixtures, including final surface after smoothing. |
...
| `surfaceVoting(...)` | `_surface_voting` | For one seed, builds local axes, samples the UVW cost box, finds an optimal surface, computes average fault attribute and smoothed-surface strike/dip, then accumulates votes and orientation maps. | Uses `FaultCell` vector helpers, `samples_in_uvw_box`, `dp.find_surface_3d`, `_surface_vote_average`, `_surface_strike_and_dip` with `surface_orientation_smoothing`, and `_accumulate_surface_votes`. | Several substeps are approximations; Java parallelism and in-place updates can expose ordering differences; full method parity is not established. Current reference-audit tests cover the average, smoothing wiring, orientation helper, and accumulation helpers, not the complete single-seed workflow. | `partially covered` | Add a full single-seed fixture that compares local costs, picked surface, average value, accumulated vote footprint, and orientation maps. |
...
| --- | --- | --- |
| `FaultOrientScanner3.thin(...)` smooths only the `i3` and `i2` axes, applies strike-binned strict local maxima in the `i2-i3` plane, writes retained smoothed likelihoods with retained input strike/dip, assigns Java `NO_STRIKE` / `NO_DIP` to rejected orientations, then calls `removeEdgeEffects(...)`. | `FaultOrientScanner3.thin(mode="reference")` uses `reference_like_3d_thin_values(..., reinforce_vertical=False)` and returns zero orientation sentinels at rejected samples. It does not currently apply scanner edge-effect removal. | Edge-effect removal is scanner-specific in the Java reference. If implemented later, share code only as an internal helper and call it from scanner-compatible paths explicitly; do not apply it to voter thinning by default. Keep Python's public zero orientation sentinel for compatibility instead of introducing Java `-0.00001` sentinels into returned arrays. |
| `FaultOrientScanner3.removeEdgeEffects(...)` zeros likelihood, strike, and dip within five samples of the `i3` faces when the squared fault-normal `w3` component exceeds `cos(30 deg)^2`, and within five samples of the `i2` faces when `w2^2` exceeds the same threshold. It does not remove `i1` face samples. | No matching public Python edge-effect helper is wired into `orient3d` or `thinning3d` in this audit step. | Future parity work should treat this as a post-thinning scanner cleanup, not as part of the shared NMS mask. Tests should cover `i2` and `i3` faces separately and leave `i1` faces unaffected. |
| `OptimalSurfaceVoter.thin(...)` uses the same strike-binned NMS structure but does not call `removeEdgeEffects(...)`. It writes Java sentinels only at rejected orientations and includes the voter-only neighbor write for retained near-vertical folded strikes; the audited Java horizontal condition is unreachable as written (`p000 < 30 && p000 > 150`). | `OptimalSurfaceVoter.thin(mode="reference")` calls the shared helper with `reinforce_vertical=True`; default `mode="normal"` remains the legacy Python normal-vector thinning path. The public method returns only thinned values, so Java rejected-orientation sentinels are not exposed. | Keep scanner and voter reference-like thinning as separate call sites over the shared helper because the voter has reinforcement behavior and no edge-effect removal. Do not change the voter default in this audit-only step. |
| `OptimalSurfaceVoter.surfaceVoting(...)` accepts vote surface points only when `0 <= i1 < n1`, `0 < i2 < n2 - 1`, and `0 < i3 < n3 - 1`; neighbor reinforcement then writes `i2 +/- 1` or `i3 +/- 1` without additional edge checks. `samplesInUvwBox(...)` and `screenPoints(...)` use the same interior `i2`/`i3` condition for their in-box checks. | `_surface_vote_average_python/_numba` and `_accumulate_surface_votes_python/_numba` currently accept all boundary samples satisfying `0 <= i1 < n1`, `0 <= i2 < n2`, and `0 <= i3 < n3`; `_add_surface_vote` also bounds-checks reinforced neighbors. | Future reference-mode surface voting should move the target-point predicate to Java's `i1`-inclusive, `i2`/`i3`-interior rule. Keeping `_add_surface_vote` bounds checks is still harmless defensive behavior, but the average and accumulation point sets should be narrowed together. |
...
- Surface-voting boundaries. Java permits all in-range `i1` samples but
  ...
  current Python voting helpers still include those faces.
