...
| surface-orientation smoothing config | `set_surface_orientation_smoothing` / `surface_orientation_smoothing` | Constructor creates a recursive Gaussian filter with `max(rv, rw)` and applies it to the extracted surface before strike/dip are recomputed. | Defaults to `float(max(rv, rw))`, validates nonnegative finite values, and passes the value to `_surface_strike_and_dip`; `0.0` disables orientation-only surface smoothing. | Python stores a scalar instead of a filter object and approximates the smoothing with `dp.smooth_surface_2d`; edge response is not expected to be bit-exact. | `python-regression covered` | Compare noisy and stair-step Java-derived surfaces before/after smoothing to verify practical orientation-jitter reduction and sign conventions. |
...
- Surface orientation smoothing. Python smooths extracted `(nw, nv)` surfaces
  ...
  `surface_orientation_smoothing > 0.0`; Java recursive Gaussian behavior and
  boundary handling need direct fixtures.
