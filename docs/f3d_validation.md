...
The matching F3 shared-scan policy validation and manual review are still
pending, and no quality-workflow or public scanner-thinning default changes as
a result of the synthetic pass alone.
...
F3 evidence fixture before the real runs and human review have occurred. The F3
shared-scan evidence is currently pending, so no such fixture is committed yet.
...
- Do not commit generated PNGs or full F3 report JSON/Markdown. Commit only an
  explicitly documented compact evidence manifest after the required run and
  review.
